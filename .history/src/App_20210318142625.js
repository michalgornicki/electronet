...
class App extends Component {
  ...
  AddToBasket = event => {
    this.setState({
      ...
      myBasket: [...this.state.myBasket, {
        ...
        price: event.target.parentElement.getElementsByClassName('price')[0].innerHTML,
  ...
  AddToBasketCart = event => {
    this.setState({
      ...
      basketItems: this.state.basketItems + 1,
      myBasket: [...this.state.myBasket, {title: event.target.parentElement.getElementsByClassName('cartTitle')[0].innerHTML, price: event.target.parentElement.getElementsByClassName('cartPrice')[0].innerHTML}]
    });
}
  ...
  return (
    ...
    <h1 className="main-header"> <a href="index.html"><div className="main-header-text">ELECTRONET</div></a> <form className="search-product" action=""> <input className="search-product-input" type="text" placeholder="Znajdź produkt" onChange={console.log("aaa")}/><div className="search-product-button" type="submit" onClick={this.searchProduct}>SZUKAJ</div></form> <div className="basket" onClick={this.clickBasket}><img className="cart-large" src="cart.png" alt=""/> ({this.state.basketItems}) {this.state.myBasket.price.reduce((pv, cv) => pv + cv, 0)} zł</div> </h1>
            ...
            <div className="basket-items">
              ...
              .map(basketItem =>{
                return(
                  ...
                  <div className="basket-item-price"> {basketItem.price}</div> 
