  ...
  return (
    <div className="items-container">
      ...
      {data
        ...
        .filter((element) =>
          props.StateData.category.includes(element.category)
        )

        .filter(
          (element) =>
            parseFloat(element.price) < props.StateData.maxPrice &&
            parseFloat(element.price) > props.StateData.minPrice
        )

        .filter((element) =>
          element.title.toLowerCase().includes(props.StateData.searchInput)
        )
