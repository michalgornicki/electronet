...
const ItemList = () => {
    ...
    return(
        ...
        .filter(element => itemData.category.includes(element.category))

        .filter(element => parseFloat(element.price) < itemData.maxPrice && parseFloat(element.price) > itemData.minPrice)
        
        .filter(element => element.title.toLowerCase().includes(itemData.searchInput))

        .sort((a, b) => {
          if (itemData.priceSort === "ascending")
