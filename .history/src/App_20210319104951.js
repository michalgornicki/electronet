...
class App extends Component {
  ...
  deleteItem = event => {
    console.log(event.target.parentElement.getElementsByClassName("basket-item-index")[event.target.parentElement.getElementsByClassName("basket-item-index").innerHTML])
    this.setState({
      myBasket: this.state.myBasket.slice(event.target.parentElement.getElementsByClassName("basket-item-index")[event.target.parentElement.getElementsByClassName("basket-item-index").innerHTML], 1),
    })
  }
  ...
  return (
    ...
    <div className="basket" onClick={this.clickBasket}>
    <img className="cart-large" src="cart.png" alt=""/> ({this.state.basketItems}) {this.state.myBasket.reduce((acc, array) => acc + array.price, 0).toFixed(2)}  zł
    </div> 
    </h1>
