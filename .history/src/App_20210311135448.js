...
class App extends Component {
  ...
  AddToBasket = event => {
    this.setState({basket: this.state.basket + event.target.parentElement});
    console.log(event.target.id);
  }
  ...
  return (
    ...
    <div>
        ...
        <div className="item" key={data.id}>
          ...
          <h2>{product.price}</h2>
          <div className="add-to-cart" onClick={AddToBasket}><img className="cart" src="cart.png" alt=""/>Do koszyka</div>
        </div>
        )})
        }
        </div>
